...
def get_next_empty_right(sites: dict[int, Site], my_queen: Unit):
    empty_right_sites = [site for site in sites.values() if site.type == SiteType.EMPTY and site.pos[0] > my_queen.pos[0]]
    if empty_right_sites:
        empty_right_sites = sorted(empty_right_sites, key=lambda site: math.dist(my_queen.pos, site.pos))
        return empty_right_sites[0]
    else:
        return None

def get_next_empty_left(sites: dict[int, Site], my_queen: Unit):
    empty_left_sites = [site for site in sites.values() if site.type == SiteType.EMPTY and site.pos[0] < my_queen.pos[0]]
    if empty_left_sites:
        empty_left_sites = sorted(empty_left_sites, key=lambda site: math.dist(my_queen.pos, site.pos))
        return empty_left_sites[0]
    else:
        return None

def get_build_string(
        closest_empty_site: Site | None, 
        friendly_sites: FriendlySites, 
        enemy_towers, 
        are_enemy_knights_close,
        are_enemy_knights_left,
        sites,
        my_queen) -> str:
    ...
    if are_enemy_knights_close:
        if len(my_towers) >= 2:
            return f"BUILD {friendly_sites.find_outest_tower().id} TOWER"
        else:
            if are_enemy_knights_left:
                next_empty_right = get_next_empty_right(sites, my_queen)
                if next_empty_right:
                    return f"BUILD {next_empty_right.id} TOWER"
                else:
                    return f"MOVE 1920 540"
            else:
                next_empty_left = get_next_empty_left(sites, my_queen)
                if next_empty_left:
                    return f"BUILD {next_empty_left.id} TOWER"
                else:
                    return f"MOVE 0 540"
...
def are_enemy_knights_left(pos: list[int], enemy_knights: list[Unit]) -> bool:
    counter_left = 0
    counter_right = 0;
    for knight in enemy_knights:
        if knight.pos[0] <= pos[0]:
            counter_left += 1
        else:
            counter_right += 1
...
while True:
    ...
    build_string = get_build_string(
        closest_empty_site, 
        friendly_sites, 
        enemy_towers, 
        are_enemy_knights_close(my_queen.pos, enemy_knights), 
        are_enemy_knights_left(my_queen.pos, enemy_knights),
        sites,
        my_queen)
    print(build_string)
